<?php
    ...
    class ErrorController extends \Exception{
        ...
        static public function setError(string $message,object $errors,int $type){
            ...
            self::createErrorLog($errorFormated);
        }
        ...
        static public function createErrorLog(array $error):void{
            $day = date("d");
            $week = date("W");
            $month = date("m");
            $year = date("Y");
            ...
            $errorToString = "[".date('m-d-Y_h:i:s', time())."][". $_SERVER["REMOTE_ADDR"] . "]-" . $error['message']. "-" . $error['file']. "-Line:" . $error['line'];
